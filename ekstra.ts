...
class Ekstra {
     ...
     * @param {string} ch2
     */
    static encryptWord(msg: string, key: string, ch1: string, ch2: string) {

        Ekstra.#validateInput(msg,key,ch1,ch2,true);

        const OldBase = BigInt(ch1.length),
              NewBase = BigInt(ch2.length),
              enigma = new Ekstra.#Enigma(ch1.length);
     ...
     * @param {string} ch2
     */
    static decryptWord(msg: string, key: string, ch1: string, ch2: string) {

        Ekstra.#validateInput(msg,key,ch1,ch2,false);

        const OldBase = BigInt(ch1.length),
              NewBase = BigInt(ch2.length),
              enigma = new Ekstra.#Enigma(ch2.length);
    ...
    static randomizeCharset(charset:string) {
        let t = charset, o = "";
        while (o.length !== charset.length) {
            const r = Math.floor(Math.random() * t.length);
            o += t[r];
    ...
    static #getSubstrings(fixedSize:boolean,msgLength:number,substringLength:number) {
        if (!fixedSize&&(!substringLength||substringLength<4||!Number.isInteger(substringLength))) throw "fixeSize must be an integer higher or equal to 3!";
        const o = [] as number[];
    ...
    static encryptPhrase(msg:string, key:string, ch1:string, ch2:string, fixedSize:boolean, substringLength:number, inBetween=" "as string|string[]) {
        Ekstra.#validateInput(msg,key,ch1,ch2,true);
        if (typeof fixedSize !== "boolean") throw "no fixedSize was provided: cannot interpret whether the substrings should be of fixedSize size or not!";
        ...
        const OldBase = BigInt(ch1.length),
              NewBase = BigInt(ch2.length),
              enigma = new Ekstra.#Enigma(ch1.length),
              cuts = Ekstra.#getSubstrings(fixedSize,msg.length,substringLength);
        ...
        for (let i=0; i<cuts.length-1; i++) {
            ...
            if (i<cuts.length-2) {
                if (Array.isArray(inBetween)) o += inBetween[i%inBetween.length];
                else o += inBetween;
            }
        }
    ...
    static decryptPhrase(msg:string, key:string, ch1:string, ch2:string, inBetween:string|string[]=" ") {
    ...
    static fastEncrypt(msg:string,fixedSize:boolean,substringLength=0) {
        const ch1 = Ekstra.randomizeCharset(Ekstra.completeCharset(Ekstra.extractCharset(msg),64)),
              ch2 = Ekstra.completeCharset(Ekstra.randomizeCharset(Ekstra.hexCharset),17);
        let key = "";
        for (let i = 0; i < 15; i++)
        ...
        if (Ekstra.decryptPhrase(encr,key,ch2.slice(0,16),ch1,ch2[16])!==msg) return Ekstra.fastEncrypt(msg,fixedSize,substringLength)
